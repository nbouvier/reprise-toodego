...
export function signURL(_url) {
    ...
    let query = `${(parsedURL.search ? `${parsedURL.search}&`.substr(1) : '')}algo=sha256&timestamp=${timestamp}&nonce=${nonce}&orig=${process.env.TOODEGO_USER}`;
...
function signHmacSha256(_str) {
    let hmac = crypto.createHmac('sha256', process.env.TOODEGO_KEY);
